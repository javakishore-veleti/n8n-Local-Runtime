...
Generate a **single HTML file** containing **an Introduction page plus 9 diagrams** (10 pages total), each on its own full-height page section. Use a dark theme with a sticky top navigation bar to jump between pages. The first nav link should be "Introduction" followed by the 9 diagram links. The aesthetic should be enterprise-grade, polished, dark-mode (background ~#0a0d14), with color-coded layers and clean iconography.
...
### The 10 Pages (Introduction + 9 Diagrams)

**Introduction Page: Product Overview**
- Hero section with SkillForge name, tagline pill badge "AI-POWERED TEST VALIDATION PLATFORM", subtitle
- Two-column grid: Vision card (blue gradient top border) and Objective card (green gradient top border)
- The Problem section: 4-card grid showing pain points — Manual & Slow (🐌), Trillion Combinations (🧠), Knowledge Silos (🔒), No Self-Service (🚫). Each with emoji icon, title, description. Red-toned cards.
- The Solution section: 3-card grid — Skills (📝, blue), Smart Execution (🤖, purple), Business Results (📊, green). Each with centered icon in a rounded box, title, description.
- Two-column grid: Key Principles card (4 numbered principles: SKILL.md is the contract, Author once execute infinitely, Credentials never exposed, 95/5 execution model) and "What It Feels Like" card (4 numbered user steps: Pick skill → Fill criteria → Click Run → Read results, plus a quoted example scenario in a green-toned callout box)
- Document Guide section: 3-column grid advising different audiences (Executives, Architects, QA/Business Users) which diagrams to focus on, with colored left borders
...
- Sticky navigation bar at top with links to each page section — Introduction first, then all 9 diagrams
- The Introduction page uses only CSS/HTML cards and grids (no SVG needed)
...
- Print-friendly: page breaks between pages
- Total: 10 pages (1 Introduction + 9 Diagrams)

Generate the complete HTML file now with all 10 pages (Introduction + 9 diagrams).
